...
We know that adaptive A* increases the values of $h$ as the algorithm is executed to include the new cells that have been discovered and are blocked. This allows it to decide to expand cells that may have greater manhattan distance than other cells with the same $g$ value because the other cell's $h$ value was increased with the adaptive A*. This increase occurrs because the previous searches have discovered that the actual distance from that cell to the target is greater than its manhattan distance given the current explored cells in the board.
